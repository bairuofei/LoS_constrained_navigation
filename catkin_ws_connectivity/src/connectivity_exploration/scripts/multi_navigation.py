...
class Controller:
    def __init__(self, num_robot: int, enable_record: bool, save_folder: str, enable_mst: bool = False, world_name: str = "empty", exp_suffix: str = ""):
        ...
        self.robot_trajectories = [[] for _ in range(self.num_robot)]
        ...
        self.has_received_robots_initial_positions = True  # as the initial positions are set in yaml file, no need to wait for the gazebo modelStates
        ...
        self.pub_robot_trajectories = rospy.Publisher("/robot_trajectories", MarkerArray, queue_size=2)
        ...
        self.robots_reach_target = [False for _ in range(self.num_robot)]
    ...
    def save_data_when_exit(self):
        ...
        if self.recorder.stop_time < 0:   # if navigation not stopped
            ...
            print(f"{RED}Navigation unfinished after {self.recorder.get_running_time()} seconds!{RESET}")
        else:
            print(f"{RED}Navigation finished after {self.recorder.get_running_time()} seconds!{RESET}")
    ...
    def callback_flipped_convexhull(self, msg: Float32MultiArray, robot_id: int):
        """_summary_
    ...
    def callback_waypoints(self, data: PointStamped, robot_id: int):
        ...
        self.robots[robot_id].set_waypoint((data.point.x, data.point.y))  # Set intermidiate waypoint as controller's target
    ...
    def publish_robot_trajectories(self):
        marker_array = MarkerArray()

        for r in range(self.num_robot):
            marker = Marker()
            marker.header.frame_id = "odom"  # Adjust to your frame
            marker.header.stamp = rospy.Time.now()
            marker.ns = f"trajectories_{r}"
            marker.id = r  # Unique ID for each marker
            ...
            for waypoint in self.robot_trajectories[r]:
                point = Point()
                point.x = waypoint[0]
                point.y = waypoint[1]
                point.z = 0
        ...
        self.pub_robot_trajectories.publish(marker_array)
        return
    ...
    def set_robot_and_target_positions(self):
        ...
        for r in data["robot_list"]:
            ...
            target_position = data[f"robot_{r}"]["target_position"]
            self.robots[r].set_pose(tuple(self_position))
            self.set_robot_modelState(r, tuple(self_position))
            if len(target_position) > 0:
                self.robots[r].set_target(tuple(target_position), force_reset=True)
                self.robots[r].set_waypoint(tuple(target_position))
                self.publish_target_to_far_planner(r, tuple(target_position))
    ...
    def main_loop(self, general_lp: GeneralizedLaplacian):
        ...
        timer_robots_stay: Dict[int, int] = {} # Timer records (robot, remain_steps)
        stay_steps = 3 * loop_rate  # 3 seconds
        ...
        while not rospy.is_shutdown():
            ...
            if not is_graph_connected:
                if self.stopMove:  # FIXME: This is where error occurs
            ...
            self.timer.tic()
            # Leading robot is the closest to its target
            # (8.1) Get list of robots that have targets
            robots_have_targets = set()
            str_robots_have_targets = "robots_with_targets: "
            for r in range(self.num_robot):
                if self.robots[r].has_a_target() and r not in timer_robots_stay and not self.robots_reach_target[r]:  # Robots have new targets and not need to stay
                    robots_have_targets.add(r)
                    str_robots_have_targets += str(r+1) + " "
            my_print(str_robots_have_targets, enable_print)
            if len(robots_have_targets) == 0:
                self.has_leading_robot = False

            # (8.2) Update leading robot
            # a. elect a new leading_robot, when there is no leading_robot or leading_robot reaches target
            # b. else leading_robot remains
            if not self.has_leading_robot or self.robots[self.leading_idx].has_reach_target():  # Current leading robot has reached target
                if self.has_leading_robot:  # Means robot has reach targets
                    self.robots_reach_target[self.leading_idx] = True
                    self.robots[self.leading_idx].clear_target()
                    robots_have_targets.remove(self.leading_idx)
                    self.has_leading_robot = False
                    timer_robots_stay[self.leading_idx] = stay_steps

                # Select a new leading robot with the smallest distance to its target
                if len(robots_have_targets) == 0:
                    self.has_leading_robot = False
                else:
                    min_target_dist = float("inf")
                    min_robot_idx = -1
                    for r in robots_have_targets:
                        dist_to_target = self.robots[r].get_dist_to_target()
                        if dist_to_target < min_target_dist:
                            min_target_dist = dist_to_target
                            min_robot_idx = r
                    self.has_leading_robot = True   # Set new leading robot
                    self.leading_idx = min_robot_idx  
            ...
            for r, pose in enumerate(robot_positions):
                ...
                # self.robot_trajectories[r].append(pose)

            #FIXME: when publish trajectories to rviz, the controller frequency is smaller
            # self.publish_robot_trajectories()
...
def main():
    ...
    robot_controller.set_robot_and_target_positions()
